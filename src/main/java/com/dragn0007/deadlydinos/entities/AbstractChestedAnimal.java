...
public abstract class AbstractChestedAnimal extends AbstractBaseDino {
   private static final EntityDataAccessor<Boolean> DATA_ID_CHEST = SynchedEntityData.defineId(AbstractChestedAnimal.class, EntityDataSerializers.BOOLEAN);

   protected AbstractChestedAnimal(EntityType<? extends AbstractChestedAnimal> p_30485_, Level p_30486_) {
   ...
   public SlotAccess getSlot(int p_149479_) {
      return p_149479_ == 499 ? new SlotAccess() {
         public ItemStack get() {
            return AbstractChestedAnimal.this.hasChest() ? new ItemStack(Items.CHEST) : ItemStack.EMPTY;
         }

         public boolean set(ItemStack p_149485_) {
            if (p_149485_.isEmpty()) {
               if (AbstractChestedAnimal.this.hasChest()) {
                  AbstractChestedAnimal.this.setChest(false);
                  AbstractChestedAnimal.this.createInventory();
               }

               return true;
            } else if (p_149485_.is(Items.CHEST)) {
               if (!AbstractChestedAnimal.this.hasChest()) {
                  AbstractChestedAnimal.this.setChest(true);
                  AbstractChestedAnimal.this.createInventory();
               }

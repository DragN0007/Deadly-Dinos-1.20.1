...
@OnlyIn(Dist.CLIENT)
public class AnurognathusOnShoulderLayer<T extends Player>
        extends RenderLayer<T, PlayerModel<T>> {

    private final AnurognathusRender geckoRenderer;
    private Anurognathus leftDummy;
    private Anurognathus rightDummy;

    public AnurognathusOnShoulderLayer(RenderLayerParent<T, PlayerModel<T>> parent, EntityRendererProvider.Context ctx) {
        super(parent);
        this.geckoRenderer = new AnurognathusRender(ctx);
    }
    ...
    private void renderOneShoulder(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T player, float partialTicks, boolean left) {
        ...
        Anurognathus dummy = left ? leftDummy : rightDummy;
        if (dummy == null) {
            dummy = new Anurognathus(EntityTypes.ANUROGNATHUS.get(), player.level());
            if (left) leftDummy = dummy; else rightDummy = dummy;
        }
        if (EntityType.byString(tag.getString("id")).filter(t -> t == EntityTypes.ANUROGNATHUS.get()).isEmpty()) {
            return;
        }

...
public class Gryposaurus extends AbstractDinoMount implements GeoEntity {

	public Gryposaurus(EntityType<? extends Gryposaurus> type, Level level) {
		super(type, level);
		noCulling = true;
	}
	...
	public void aiStep() {
		...
		if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.eggTime <= 0 && (!DeadlyDinosCommonConfig.GENDERS_AFFECT_BIPRODUCTS.get() || (DeadlyDinosCommonConfig.GENDERS_AFFECT_BIPRODUCTS.get() && this.isFemale()))) {
			this.spawnAtLocation(DDDItems.GRYPOSAURUS_EGG.get());
	...
	public ResourceLocation getFemaleTextureLocation() {
		return GryposaurusModel.FemaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public ResourceLocation getMaleTextureLocation() {
		return GryposaurusModel.MaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(Gryposaurus.class, EntityDataSerializers.INT);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor serverLevelAccessor, DifficultyInstance instance, MobSpawnType spawnType, @Nullable SpawnGroupData data, @Nullable CompoundTag tag) {
		...
		if (this.isFemale()) {
			setVariant(random.nextInt(GryposaurusModel.FemaleVariant.values().length));
		} else if (this.isMale()) {
			setVariant(random.nextInt(GryposaurusModel.MaleVariant.values().length));
		}
	...
	public boolean canMate(Animal animal) {
		if (animal == this) {
			return false;
		} else if (!(animal instanceof Gryposaurus)) {
			return false;
		} else {
			if (!DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) {
				return this.canParent() && ((Gryposaurus) animal).canParent();
			} else {
				Gryposaurus partner = (Gryposaurus) animal;
	...
	public void dropFertilizedEgg(ServerLevel serverLevel) {
		...
		if ((this.isFemale() && DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) || !DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) {
			ItemStack fertilizedEgg = new ItemStack(DDDItems.FERTILIZED_GRYPOSAURUS_EGG.get());
	...
	public void dropCustomDeathLoot(DamageSource p_33574_, int p_33575_, boolean p_33576_) {
		...
		if (this.isFemale() && eggChance <= 5) {
			this.spawnAtLocation(DDDItems.FERTILIZED_GRYPOSAURUS_EGG.get());
		}

		int trophyChance = random.nextInt(100);
		if (trophyChance <= 8) {
			this.spawnAtLocation(DDDItems.GRYPOSAURUS_TROPHY.get());
		}
	}

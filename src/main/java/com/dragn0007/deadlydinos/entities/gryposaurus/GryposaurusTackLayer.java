...
@OnlyIn(Dist.CLIENT)
public class GryposaurusTackLayer extends GeoRenderLayer<Gryposaurus> {
    public GryposaurusTackLayer(GeoRenderer<Gryposaurus> entityRendererIn) {
    ...
    public void render(PoseStack poseStack, Gryposaurus animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (animatable.isSaddled() || animatable.hasChest()) {
            resourceLocation = new ResourceLocation(DeadlyDinos.MODID, "textures/entity/gryposaurus/tack/saddle.png");
        } else {
            return;
        }

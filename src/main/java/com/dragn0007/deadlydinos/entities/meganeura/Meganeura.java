...
import com.dragn0007.deadlydinos.entities.ai.herd.MeganeuraFollowPackLeaderGoal;
...
public class Meganeura extends AbstractDino implements GeoEntity, FlyingAnimal {

	public Meganeura(EntityType<? extends Meganeura> type, Level level) {
		super(type, level);
		noCulling = false;
	...
	public void registerGoals() {
		...
		this.goalSelector.addGoal(3, new MeganeuraFollowPackLeaderGoal(this));
				...
				if (DeadlyDinosCommonConfig.INJURY_EFFECTS.get()) {
					if (i > 0 && chance <= 75) {
						if (!((LivingEntity) entity).hasEffect(DDDEffects.BUG_BITE.get())) {
							((LivingEntity) entity).addEffect(new MobEffectInstance(DDDEffects.BUG_BITE.get(), i * 20, 0, true, false, true));
						}
					}
				}
			}
	...
	public Meganeura leader;
	public int packSize = 1;

	public int getMaxHerdSize() {
		return DeadlyDinosCommonConfig.MEGANEURA_MAX_SWARM_COUNT.get();
	}
	...
	public void addFollowers(Stream<? extends Meganeura> p_27534_) {
	...
	public Meganeura startFollowing(Meganeura mob) {
		this.leader = mob;
		mob.addFollower();
		return mob;
	}
	...
	public ResourceLocation getFemaleTextureLocation() {
		return MeganeuraModel.FemaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public ResourceLocation getMaleTextureLocation() {
		return MeganeuraModel.MaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(Meganeura.class, EntityDataSerializers.INT);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor serverLevelAccessor, DifficultyInstance instance, MobSpawnType spawnType, @Nullable SpawnGroupData data, @Nullable CompoundTag tag) {
		...
		if (this.isFemale()) {
			setVariant(random.nextInt(MeganeuraModel.FemaleVariant.values().length));
		} else if (this.isMale()) {
			setVariant(random.nextInt(MeganeuraModel.MaleVariant.values().length));
		}
	...
	public boolean canMate(Animal animal) {
		if (animal == this) {
			return false;
		} else if (!(animal instanceof Meganeura)) {
			return false;
		} else {
			if (!DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) {
				return this.canParent() && ((Meganeura) animal).canParent();
			} else {
				Meganeura partner = (Meganeura) animal;
	...
	protected static final EntityDataAccessor<Byte> DATA_FLAGS_ID = SynchedEntityData.defineId(Meganeura.class, EntityDataSerializers.BYTE);

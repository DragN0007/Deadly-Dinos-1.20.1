...
@OnlyIn(Dist.CLIENT)
public class VelociraptorOnShoulderLayer<T extends Player>
        extends RenderLayer<T, PlayerModel<T>> {

    private final VelociraptorRender geckoRenderer;
    private Velociraptor leftDummy;
    private Velociraptor rightDummy;

    public VelociraptorOnShoulderLayer(RenderLayerParent<T, PlayerModel<T>> parent, EntityRendererProvider.Context ctx) {
        super(parent);
        this.geckoRenderer = new VelociraptorRender(ctx);
    }
    ...
    private void renderOneShoulder(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T player, float partialTicks, boolean left) {
        ...
        Velociraptor dummy = left ? leftDummy : rightDummy;
        if (dummy == null) {

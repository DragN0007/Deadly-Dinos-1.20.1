...
public class Parasaurolophus extends AbstractDinoMount implements GeoEntity {
	...
	public Parasaurolophus(EntityType<? extends Parasaurolophus> type, Level level) {
		super(type, level);
		noCulling = true;
	}
	...
	public ResourceLocation getFemaleTextureLocation() {
		return ParasaurolophusModel.FemaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public ResourceLocation getMaleTextureLocation() {
		return ParasaurolophusModel.MaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(Parasaurolophus.class, EntityDataSerializers.INT);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor serverLevelAccessor, DifficultyInstance instance, MobSpawnType spawnType, @Nullable SpawnGroupData data, @Nullable CompoundTag tag) {
		...
		if (this.isFemale()) {
			setVariant(random.nextInt(ParasaurolophusModel.FemaleVariant.values().length));
		} else if (this.isMale()) {
			setVariant(random.nextInt(ParasaurolophusModel.MaleVariant.values().length));
		}
	...
	public boolean canMate(Animal animal) {
		if (animal == this) {
			return false;
		} else if (!(animal instanceof Parasaurolophus)) {
			return false;
		} else {
			if (!DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) {
				return this.canParent() && ((Parasaurolophus) animal).canParent();
			} else {

package com.dragn0007.deadlydinos.entities.spinosaurus;
...
public class Spinosaurus extends AbstractDino implements GeoEntity {

	public Spinosaurus(EntityType<? extends Spinosaurus> type, Level level) {
		super(type, level);
		noCulling = true;
	...
	public void registerGoals() {
		...
		this.goalSelector.addGoal(5, new RandomSwimmingGoal(this, 1.5D, 1));
		...
		this.goalSelector.addGoal(1, new AvoidEntityGoal<>(this, LivingEntity.class, 15.0F, 2.0F, 1.8F,
				entity -> entity.getType().is(DDDTags.Entity_Types.LARGE_DINOS_RUN_FROM)));
		...
		this.goalSelector.addGoal(2, new DinoNearestAttackableTargetGoal<>(this, LivingEntity.class, 3, true, false,
				entity -> entity.getType().is(DDDTags.Entity_Types.PREDATORS) && !entity.getType().is(DDDTags.Entity_Types.LARGE_PREDATORS) && !this.isBaby() && !(entity.getType() == (EntityTypes.SPINOSAURUS.get()))));
	...
	public void aiStep() {
		...
		if (DeadlyDinosCommonConfig.ALLOW_EGG_LAY.get()) {
			if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.eggTime <= 0 && (!DeadlyDinosCommonConfig.GENDERS_AFFECT_BIPRODUCTS.get() || (DeadlyDinosCommonConfig.GENDERS_AFFECT_BIPRODUCTS.get() && this.isFemale()))) {
				this.spawnAtLocation(DDDItems.SPINOSAURUS_EGG.get());
	...
	public <T extends GeoAnimatable> PlayState predicate(software.bernie.geckolib.core.animation.AnimationState<T> tAnimationState) {
		...
		if (isMoving) {
			if (!this.isInWater()) {
				...
				if ((currentSpeed > speedThreshold)) {
					controller.setAnimation(RawAnimation.begin().then("sprint_swim", Animation.LoopType.LOOP));
					controller.setAnimationSpeed(1.2);
				} else {
			...
			if (!this.isInWater()) {
				...
				controller.setAnimation(RawAnimation.begin().then("swim_idle", Animation.LoopType.LOOP));
				controller.setAnimationSpeed(0.8);
			}
		}
	...
	public ResourceLocation getFemaleTextureLocation() {
		return SpinosaurusModel.FemaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public ResourceLocation getMaleTextureLocation() {
		return SpinosaurusModel.MaleVariant.variantFromOrdinal(getVariant()).resourceLocation;
	}

	public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(Spinosaurus.class, EntityDataSerializers.INT);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor serverLevelAccessor, DifficultyInstance instance, MobSpawnType spawnType, @Nullable SpawnGroupData data, @Nullable CompoundTag tag) {
		...
		if (this.isFemale()) {
			setVariant(random.nextInt(SpinosaurusModel.FemaleVariant.values().length));
		} else if (this.isMale()) {
			setVariant(random.nextInt(SpinosaurusModel.MaleVariant.values().length));
		}
	...
	public boolean canMate(Animal animal) {
		if (animal == this) {
			return false;
		} else if (!(animal instanceof Spinosaurus)) {
			return false;
		} else {
			if (!DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) {
				return this.canParent() && ((Spinosaurus) animal).canParent();
			} else {
				Spinosaurus partner = (Spinosaurus) animal;
	...
	public void dropFertilizedEgg(ServerLevel serverLevel) {
		...
		if ((this.isFemale() && DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) || !DeadlyDinosCommonConfig.GENDERS_AFFECT_BREEDING.get()) {
			ItemStack fertilizedEgg = new ItemStack(DDDItems.FERTILIZED_SPINOSAURUS_EGG.get());
	...
	public void dropCustomDeathLoot(DamageSource p_33574_, int p_33575_, boolean p_33576_) {
		...
		if (this.isFemale() && this.isFemale() && eggChance <= DeadlyDinosCommonConfig.EGG_CHANCE.get()) {
			this.spawnAtLocation(DDDItems.FERTILIZED_SPINOSAURUS_EGG.get());
		}

		int trophyChance = random.nextInt(100);
		if (trophyChance <= DeadlyDinosCommonConfig.TROPHY_CHANCE.get()) {
			this.spawnAtLocation(DDDItems.SPINOSAURUS_TROPHY.get());
		}
	}

	static class SwimMoveControl extends MoveControl {
		private final Spinosaurus fish;

		SwimMoveControl(Spinosaurus p_27501_) {
			super(p_27501_);
			this.fish = p_27501_;
		}

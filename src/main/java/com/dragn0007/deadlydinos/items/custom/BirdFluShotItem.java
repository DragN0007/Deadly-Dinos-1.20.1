...
public class BirdFluShotItem extends Item {

    public BirdFluShotItem(Properties properties) {
        super(properties);
    }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        ...
        if (DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() <= 0.0) {
            cureChanceText = "No";
        } else if (DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() > 0.0 && DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() <= 0.25) {
            cureChanceText = "Low";
        } else if (DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() > 0.25 && DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() <= 0.75) {
            cureChanceText = "Average";
        } else if (DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() > 0.75 && DeadlyDinosCommonConfig.ANTI_FLU_SUCCESS_CHANCE.get() <= 1.0) {
            cureChanceText = "High";
        }

        if (DeadlyDinosCommonConfig.MEDICAL_SUPPLIES.get()) {
            pTooltipComponents.add(Component.translatable(cureChanceText + " chance of success at curing Avian Influenza.").withStyle(ChatFormatting.GRAY));
        } else {

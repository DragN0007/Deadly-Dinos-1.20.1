...
import com.dragn0007.deadlydinos.entities.parasaurolophus.ParasaurolophusModel;
...
public class ThrownParasaurolophusEgg extends ThrowableItemProjectile {
   public ThrownParasaurolophusEgg(EntityType<? extends ThrownParasaurolophusEgg> p_37473_, Level p_37474_) {
      super(p_37473_, p_37474_);
   }

   public ThrownParasaurolophusEgg(Level p_37481_, LivingEntity p_37482_) {
      super(EntityType.EGG, p_37482_, p_37481_);
   }

   public ThrownParasaurolophusEgg(Level p_37476_, double p_37477_, double p_37478_, double p_37479_) {
      ...
      if (!this.level().isClientSide) {
         ...
         for (int j = 0; j < i; ++j) {
            if (getItem().is(DDDItems.FERTILIZED_PARASAUROLOPHUS_EGG.get())) {
               ...
               if (dino.isFemale()) {
                  dino.setVariant(random.nextInt(ParasaurolophusModel.FemaleVariant.values().length));
               } else if (dino.isMale()) {
                  dino.setVariant(random.nextInt(ParasaurolophusModel.MaleVariant.values().length));
               } else {
                  dino.setVariant(0);
               }
   ...
   public Item getDefaultItem() {
      return DDDItems.FERTILIZED_PARASAUROLOPHUS_EGG.get();
   }
}
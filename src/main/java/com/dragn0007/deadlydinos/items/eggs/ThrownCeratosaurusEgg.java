...
import com.dragn0007.deadlydinos.entities.ceratosaurus.CeratosaurusModel;
      ...
      if (!this.level().isClientSide) {
         ...
         for (int j = 0; j < i; ++j) {
            if (getItem().is(DDDItems.FERTILIZED_CERATOSAURUS_EGG.get())) {
               ...
               if (dino.isFemale()) {
                  dino.setVariant(random.nextInt(CeratosaurusModel.FemaleVariant.values().length));
               } else if (dino.isMale()) {
                  dino.setVariant(random.nextInt(CeratosaurusModel.MaleVariant.values().length));
               } else {
                  dino.setVariant(0);
               }

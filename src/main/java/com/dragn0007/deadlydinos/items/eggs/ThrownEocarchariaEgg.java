...
import com.dragn0007.deadlydinos.entities.eocarcharia.EocarchariaModel;
...
public class ThrownEocarchariaEgg extends ThrowableItemProjectile {
   public ThrownEocarchariaEgg(EntityType<? extends ThrownEocarchariaEgg> p_37473_, Level p_37474_) {
      super(p_37473_, p_37474_);
   }

   public ThrownEocarchariaEgg(Level p_37481_, LivingEntity p_37482_) {
      super(EntityType.EGG, p_37482_, p_37481_);
   }

   public ThrownEocarchariaEgg(Level p_37476_, double p_37477_, double p_37478_, double p_37479_) {
   ...
   public void onHit(HitResult p_37488_) {
      ...
      if (!this.level().isClientSide) {
         ...
         for (int j = 0; j < i; ++j) {
            if (getItem().is(DDDItems.FERTILIZED_EOCARCHARIA_EGG.get())) {
               Eocarcharia dino = EntityTypes.EOCARCHARIA.get().create(this.level());
               dino.setAge(-24000);
               ...
               if (dino.isFemale()) {
                  dino.setVariant(random.nextInt(EocarchariaModel.FemaleVariant.values().length));
               } else if (dino.isMale()) {
                  dino.setVariant(random.nextInt(EocarchariaModel.MaleVariant.values().length));
               } else {
                  dino.setVariant(0);
               }

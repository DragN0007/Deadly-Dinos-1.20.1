...
import com.dragn0007.deadlydinos.entities.mei_long.MeiLongModel;
      ...
      if (!this.level().isClientSide) {
         ...
         for (int j = 0; j < i; ++j) {
            if (getItem().is(DDDItems.FERTILIZED_MEI_LONG_EGG.get())) {
               ...
               if (dino.isFemale()) {
                  dino.setVariant(random.nextInt(MeiLongModel.FemaleVariant.values().length));
               } else if (dino.isMale()) {
                  dino.setVariant(random.nextInt(MeiLongModel.MaleVariant.values().length));
               } else {
                  dino.setVariant(0);
               }
